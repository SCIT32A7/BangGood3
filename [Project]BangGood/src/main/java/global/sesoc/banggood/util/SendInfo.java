...
public class SendInfo
{
    ...
    public SendInfo(String user,String title,String message) 
    {
       this.user = user;
    ...
    public void sendingid(String user, String id)
    {
    	String title = "[Ro-do] 회원 아이디";
    	String message = "아이디 : " + id;
    	new SendMail(user, title, message);
    }
    
    public int sendingpw(String user, String id)
    {
    	int accreditation = (int) (Math.random() * 1000000);
    	
    	String title = "[Ro-do] 임시 비밀번호";
    	String message = id + "님 " + "임시 비밀번호 : " + accreditation;
    	new SendMail(user, title, message);
